...
class CNN2(Pruneable):
    def __init__(self, device='cpu', in_channels=3, hidden_channels=32, num_hiddens=512, num_classes=10):
        ...
        self.conv1 = self.Conv2d(in_channels=in_channels, out_channels=hidden_channels, kernel_size=(5, 5), padding=1, stride=1, bias=True)
        self.conv2 = self.Conv2d(in_channels=hidden_channels, out_channels=hidden_channels * 2, kernel_size=(5, 5), padding=1, stride=1, bias=True)
        
        # self.maxpool1 = nn.MaxPool2d(kernel_size=(2, 2), padding=1)
        # self.maxpool2 = nn.MaxPool2d(kernel_size=(2, 2), padding=1)
        # self.flatten = nn.Flatten()

        self.fc1 = self.Linear(input_dim=(hidden_channels * 2) * (8 * 8), output_dim=num_hiddens, bias=True)
        self.fc2 = self.Linear(input_dim=num_hiddens, output_dim=num_classes, bias=True)
