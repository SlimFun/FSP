...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, testidx=None):
    return get_dataloader_CIFAR10(datadir, train_bs, test_bs, dataidxs, testidx)
...
def load_partition_data_cifar10(dataset, data_dir, partition_method, partition_alpha, client_number, batch_size):
    X_train, y_train, X_test, y_test, net_dataidx_map, traindata_cls_counts, net_testidx_map = partition_data(dataset,
                                                                                             data_dir,
                                                                                             partition_method,
                                                                                             client_number,
                                                                                             partition_alpha)
    ...
    # with open(f'dataidx.txt', 'r') as f:
    #     masks_str = f.readline()
    # #     print(masks_str)
    #     net_dataidx_map = json.loads(masks_str)
    ...
    for client_idx in range(client_number):
        dataidxs = np.array(net_dataidx_map[client_idx])
        local_data_num = len(dataidxs)
        data_local_num_dict[client_idx] = local_data_num
        logging.info("client_idx = %d, local_sample_number = %d" % (client_idx, local_data_num))

        testidx = np.array(net_testidx_map[client_idx])
        # print(np.max(testidx))

        # training batch size = 64; algorithms batch size = 32
        train_data_local, test_data_local = get_dataloader(dataset, data_dir, batch_size, batch_size,
                                                 dataidxs, testidx=testidx)
