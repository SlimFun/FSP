...
for server_round in tqdm(range(args.rounds)):
    ...
    for client_id in client_indices:
        ...
        t0 = time.time()
        train_result = client.train(global_params=global_params, initial_global_params=initial_global_params, sparsity=1-args.keep_ratio)
    ...
    for name, param in aggregated_params.items():
        ...
        if name in global_model.mask.keys():
            global_model.mask[name] = torch.where(aggregated_masks[name]>=1, 1, 0)
    ...
    #     compare_model(initial_global_params, global_model.state_dict())
        ...
        #     a = global_model.mask[name].view(-1).numpy()
    ...
    if server_round % args.eval_every == 0:
        ...
        train_accuracy, train_losses = test_model(global_model, devices[0], clients[0].train_data)
        wandb.log({"Train/Acc": train_accuracy}, step=server_round)
        wandb.log({"Train/Loss": train_losses}, step=server_round)
        print(f'round: {server_round}')
        print(f'Train/Acc : {train_accuracy}; Train/Loss: {train_losses};')
        ...
        test_accuracies, test_loss = test_model(global_model, devices[0], clients[0].test_data)
        wandb.log({"Test/Acc": test_accuracies}, step=server_round)
        wandb.log({"Test/Loss": test_loss}, step=server_round)
        print(f'Test/Acc : {test_accuracies}; Test/Loss: {test_loss};')
